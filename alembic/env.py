...
import backend.infrastructure.db.models  # noqa: F401  导入 ORM 模型以注册 metadata
...
# 优先使用环境变量 DATABASE_URL（与 backend.config 一致），
# 避免 alembic.ini 硬编码凭据；未设置时回退到 ini 中的 sqlalchemy.url。
if os.getenv("DATABASE_URL"):
    config.set_main_option("sqlalchemy.url", os.getenv("DATABASE_URL"))  # type: ignore[arg-type]
...
from backend.infrastructure.db.database import Base  # noqa: E402
