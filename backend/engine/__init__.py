...
#   solver.solve(cost_mat, initial_solution=None) -> dict                                  VNS 主循环入口
...
#   solver.solve(cost_mat) -> dict                                       CA 主循环入口
#   CA_DEFAULT_PARAMS -> 默认参数字典                                      含压缩系数、退火参数、早退阈值等
#
# ---- fitness.py ----
# analyze_solution(solution, cost_mat, spots_dict, travel_speed, ...) -> Tuple  解析路径的详细成本与违规信息
#
# ---- clustering.py ----
# CLUSTER_METHODS -> List[Tuple[str, callable]]                           6 种聚类方法注册表
# call_cluster(func, spots, depot, k, cost_mat=None) -> List[List[int]]   统一调用聚类方法
# find_method_func(name) -> callable | None                               按名称查找聚类函数
#
# ---- search.py ----
# cluster_and_solve(spots, depot, cost_mat, mode="fast", n_days=None, use_real_time_matrix=False, ...) -> dict  双阶段路由入口
# solve_groups(groups, spots, cost_mat, solver_type="CA", use_real_time_matrix=False, ...) -> dict              对已分组路径逐一求解
# ca_suggest(spots, depot, cost_mat, min_days=None, max_days=None, use_real_time_matrix=False, ...) -> dict     全参数搜索，输出 top-5 建议
