"""会话存储实现：Redis 服务端会话（实现 domain.ports.SessionStore 端口）。
...
- 语义：Redis 不可用时对认证 fail-closed——get 返回 None（视为未登录），
  create 返回 None（登录失败）。
"""
...
def _get_redis() -> Any | None:
    """懒初始化同步 Redis 客户端（decode_responses=True）。
...
class RedisSessionStore:
    """Redis 会话存储（实现 domain.ports.SessionStore 端口）。"""
