...
executor 签名统一 `(params: dict) -> dict`，纯计算不含 DB 状态流转
...
from backend.typedefs import PoiCache, PoiCacheItem

__all__ = ["TASK_EXECUTORS", "_build_poi_cache"]


def _build_poi_cache(params: dict) -> PoiCache:
...
def _run_suggest(params: dict) -> dict:
    ...
    Returns:
        dict: run_planning 建议分支完整结果（type="suggestion"）。
    """
    from backend.engine.pipeline import run_planning

    # run_planning 返回 PlanResult | dict，executor 统一按 dict 处理
...
def _run_plan(params: dict) -> dict:
    ...
    Returns:
        dict: run_planning 求解分支完整结果（type="solution"）。
    """
    from backend.engine.pipeline import run_planning
