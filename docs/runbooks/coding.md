...
`backend/utils/decorators.py` 提供三个开发层标记装饰器，统一发出 `UserWarning`
（默认可见，误调用时能立刻感知）。三者表达一个标记块的「生命周期状态」，
语义靠标记 + 被标记函数 docstring 承载，不走运行时逻辑。

| 标记 | 重在 | 语义 | 移除条件 |
|------|------|------|----------|
| `@legacy_only` | 要清理 | 已废弃、不再维护，仅作参考保留 | 确认无调用方且无保留价值 → 删函数与标记 |
| `@placeholder` | 未激活/未明确 | 存在但暂不激活，去向未定（待接线、待重构但无方向、删除等皆可能） | 接入真实调用方或方向明确 → 按新状态改标/移除；确定不要 → 删除 |
| `@refactor` | 要重构 | 实现可用但结构需重写，重构方向已明确（写在被标记函数 docstring，以 `TODO(重构方向)` 醒目标注） | 重构完成、新实现替换后 → 移除标记 |

**生命周期三步流程**：

1. **新增标记**：`@legacy_only` / `@placeholder` 只需标注即可；`@refactor` 必须在被标记函数 docstring 首行写 `TODO(重构方向)：...`，否则视为无效标记。
2. **状态流转**：`@placeholder` 接入真实调用方后移除；方向明确时按 `@refactor` 或直接重构处理。`@refactor` 完成重构后移除。`@legacy_only` 失去保留价值后删除。
3. **里程碑 review**：每个大功能迭代收尾或版本发布（如 beta / v1.0.0）时，系统清点一遍全部标记，逐项裁决：接线了 → 移除；重构完了 → 移除；确定不要 → 删除；方向过时 → 更新 docstring。

标记是「技术债追踪」而非运行时常驻机制：运行时警告仅防误调用，真正的清点靠里程碑 review。
