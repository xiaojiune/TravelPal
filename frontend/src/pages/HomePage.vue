      ...
      <table class="edit-table">
        ...
        <tbody>
          <tr v-for="(row, i) in editRows" :key="i" :class="{ 'row-hotel': row.isHotel }">
            ...
            <td><input :value="row.stay || ''" type="number" min="0" placeholder="请输入" class="num-input" @input="row.stay = Number(($event.target as HTMLInputElement).value) || 0" /></td>
            <td><input :value="row.expectedArrival || ''" type="number" min="0" max="1440" placeholder="请输入" class="num-input" @input="row.expectedArrival = Number(($event.target as HTMLInputElement).value) || 0" /></td>
          </tr>
        </tbody>
      </table>
