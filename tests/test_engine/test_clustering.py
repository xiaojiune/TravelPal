...
class TestClustering:
    ...
    def test_cluster_count_matches_k(self, any_dataset):
        # 参数化验证（any_dataset 覆盖 n20/n40/n60/n80/n100 五种规模），确保每种方法都精确返回 k 组
    ...
    def test_each_group_nonempty(self, n20_dataset):
        ...
        for full_name, method_func in CLUSTER_METHODS:
            groups = call_cluster(method_func, spots, depot, 4, cost_mat)
    ...
    def test_different_k_produce_different_partitions(self, n20_dataset):
        ...
        groups_k2 = call_cluster(method_func, spots, depot, 2, cost_mat)
        groups_k3 = call_cluster(method_func, spots, depot, 3, cost_mat)
